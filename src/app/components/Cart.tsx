...
const Cart: React.FC<CartProps> = ({ isOpen, handleCartToggle }) => {
  return (
    <>
      <div
        className={`fixed right-0 top-0 max-w-xs w-full h-full px-6 py-4 transition-all ease-in duration-300 transform ${
          isOpen ? "right-0" : "hidden"
        } overflow-y-auto bg-white border-l-2 border-gray-300`}
      >
        <div className="flex items-center justify-between">
          <h3 className="text-2xl font-medium text-gray-700">Your cart</h3>
          <button
            onClick={handleCartToggle}
            className="text-gray-600 focus:outline-none transition ease-in duration-300"
          >
            <svg
              className="h-5 w-5"
              fill="none"
